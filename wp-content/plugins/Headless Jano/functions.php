...
function check_api_key($response, $server, $request) {
    // Controleer of het verzoek een custom endpoint is
    $namespace = $request->get_route();
    if (
        strpos($namespace, '/wp/v2/media') !== false ||
        strpos($namespace, '/wp/v2/users') !== false ||
        strpos($namespace, '/wp/v2/themes') !== false
    ) {
        return $response; // Skip API key check voor bepaalde core endpoints
    }
    ...
    if (!is_user_logged_in()) {
        ...
        if ($api_key !== $_ENV['MY_API_KEY']) {
            return new WP_Error(
                'rest_forbidden',
                __('Unauthorized, invalid API key.'),
                ['status' => 401]
            );
        }
    }
...
add_filter('rest_pre_dispatch', 'check_api_key', 10, 3);
